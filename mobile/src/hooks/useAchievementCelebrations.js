...
import { fetchNotifications, isSameMoment, pickCelebrations, rememberCelebrations } from '@tobeatraveller/shared';
...
// on this device. Checked whenever the unread count changes.
export const useAchievementCelebrations = (userId, unreadCount) => {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchNotifications(1), getCelebratedNotifications()])
      .then(async ([{ notifications }, celebrated]) => {
        if (cancelled) return;
        ...
        await setCelebratedNotifications(rememberCelebrations(celebrated, picked.map(celebration => celebration.notificationId)));
      })
      .catch(() => {});
